...
export default function bolt(receiver) {
  ...
  app.message("mathMiddleSchool", async ({ message, say }) => {
    const pathText = message.text;

    const fileData = await supabase.storage
      .from("isem")
      .list(`isem-weekly-test/weekly-math/${pathText}`, {
        sortBy: { column: "created_at", order: "desc" },
      });
    const dataImg = fileData.data;

    const latestData = dataImg.shift();
    const filePath = latestData.name;

    const fileName = await supabase.storage
      .from("isem")
      .download(`isem-weekly-test/weekly-math/${pathText}/${filePath}`);

    const blob = fileName.data;
    const buffer = Buffer.from(await blob.arrayBuffer());

    const channelId = process.env.SLACK_MS_TESTCH_ID;
